...
class TestMetaProcessMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s3_endpoint_url = 'https://s3.eu-west-3.amazonaws.com'
    ...
    def test_update_meta_file_no_meta_file(self):
        ...
        MetaProcess.update_meta_file(
            self.s3_bucket_meta, meta_key, date_list_exp)
        # Read meta file
        ...
        proc_date_list_result = list(
            pd.to_datetime(
                df_meta_result[MetaProcessFormat.META_PROCESS_COL.value]).dt.date
        )
    ...
    def test_update_meta_file_empty_date_list(self):
        ...
        with self.assertLogs() as logm:
            result = MetaProcess.update_meta_file(
                self.s3_bucket_meta, meta_key, date_list)
    ...
    def test_update_meta_file_meta_file_ok(self):
        ...
        # Method execution
        MetaProcess.update_meta_file(
            self.s3_bucket_meta, meta_key, date_list_new)
        # Read meta file
        data = self.s3_bucket.Object(key=meta_key).get().get(
            'Body').read().decode('utf-8')
        out_buffer = StringIO(data)
    ...
    def test_return_date_list_no_meta_file(self):
        ...
        meta_key = 'meta.csv'
        # Method execution
        min_date_return, date_list_return = MetaProcess.return_date_list(self.s3_bucket_meta, first_date, meta_key)
        # Test after method execution
    ...
    def test_return_date_list_meta_file_wrong(self):
        ...
        with self.assertRaises(KeyError):
            MetaProcess.return_date_list(self.s3_bucket_meta, first_date, meta_key)
        # Cleanup after test
        self.s3_bucket.delete_objects(
            Delete={
                'Objects': [
...
if __name__ == "__main__":
    unittest.main()  # This calls first the setUp method, then all the tests methods and finally the tearDown method
