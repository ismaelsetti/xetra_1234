...
class XetraSourceConfig(NamedTuple):
    ...
    src_col_end_price: column name for last price in source
    src_col_min_price: colun name for minimum price in source
...
class XetraTargetConfig(NamedTuple):
    ...
    trg_col_ch_prev_clos: column name change to previous day's closing price in target
...
class XetraETL:
    ...
    def __init__(
        ...
        self.extract_date, self.extract_date_list = MetaProcess.return_date_list(
            self.s3_bucket_trg, self.src_args.src_first_extract_date, self.meta_key
        )
    ...
    def extract(self):
        ...
        :returns:
          data_frame: Pandas DataFrame with the extracted date
        """
    ...
    def transform_report1(self, data_frame: pd.DataFrame):
        ...
        data_frame[self.trg_args.trg_col_clos_price] = (
            data_frame.sort_values(by=[self.src_args.src_col_time])
            .groupby([self.src_args.src_col_isin, self.src_args.src_col_date])[
                self.src_args.src_col_end_price
            ]
            .transform("last")
        )
        # Renaming columns
        ...
        # Aggregating per ISIN and day -> opening price, closing price, minimum price, maximum price, traded volume
        ...
        # Change of current day's closing price compared to the previous trading day's closing price in %
        ...
        data_frame = data_frame[
            data_frame[self.src_args.src_col_date] >= self.extract_date
